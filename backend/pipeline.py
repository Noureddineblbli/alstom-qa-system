...
WEIGHTS_PATH = os.path.join(os.path.dirname(__file__), '..', 'detection', 'weights', 'best.pt')
CROPS_DIR = os.path.join(os.path.dirname(__file__), '..', 'preprocessing', 'crops')
ROW_THRESHOLD = 300
...
def run_pipeline(image_path):
    ...
    for result in yolo_results:
        for i, box in enumerate(result.boxes):
            ...
            detection = {"id": f"{class_name}_{i}", "label": class_name, "box": [x1, y1, x2, y2]}
    ...
    for row_idx, switch_row in enumerate(switch_rows):
        ...
        sticker_row = sticker_rows[row_idx] if row_idx < len(sticker_rows) else []
        ...
        for slot_idx, pair in enumerate(pairs):
            ...
            switch_crop_path = os.path.join(CROPS_DIR, f"pair_{slot_counter}_switch.jpg")
            sticker_crop_path = os.path.join(CROPS_DIR, f"pair_{slot_counter}_sticker.jpg")
...
if __name__ == "__main__":
    test_image = sys.argv[1] if len(sys.argv) > 1 else "detection/dataset/images/1b853c8f-1773397021222.jpg"
